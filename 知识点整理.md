...
- 编写合约 MyTokenMarket 实现：
  1. 【Uniswap】AddLiquidity(): 函数内部调⽤ Uniswap V2Router 添加 MyToken 与 ETH 的流动性
  2. 【Uniswap】buyToken(): ⽤户可调⽤该函数实现购买 MyToken

## w6_1 练习题

- 接上面练习题  
  3. 【Sushiswap】完成代币兑换后， 直接质押 MasterChef  
  4. 【Sushiswap】withdraw(): 从 MasterChef 提取 Token ⽅法

## w7_1 练习题

- 设计⼀个看涨期权 Token（ERC20）：
  1. 创建期权 Token 时，确认标的的价格与⾏权⽇期；
  2. 发⾏⽅法（项⽬⽅⻆⾊）：根据转⼊的标的（ETH）发⾏期权 Token；
  3. （可选）：可以⽤期权 Token 与 USDT 以⼀个较低的价格创建交易对，模拟⽤户购买期权。
  4. ⾏权⽅法（⽤户⻆⾊）：在到期⽇当天，可通过指定的价格兑换出标的资产，并销毁期权 Token
  5. 过期销毁（项⽬⽅⻆⾊）：销毁所有期权 Token 赎回标的 。

## w7_2 练习题

- 设计⼀个 通缩型 Token（ERC20）：
  1. 发⾏量每⼀年在上⼀年的基础上下降 1%；
  2. 调⽤⽅法 rebase() 进⾏通缩
  3. balanceOf() 及时反应通缩后余额的变化

## w8 练习题

- 实现⼀个通过 DAO 管理资⾦的 Treasury：
  1. 管理员可以从 Treasury 合约中提取资⾦ withdraw()
  2. 治理 Gov 合约作为管理员
  3. 通过发起提案从 Treasury 合约资⾦
